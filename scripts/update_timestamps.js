...
const updateTimestamp = db.prepare('UPDATE nodes SET timestamp = ? WHERE id = ?');
...
async function processData(extractor, rawNodes, sourceName) {
    ...
    const transaction = db.transaction(() => {
        clusters.forEach((cluster, idx) => {
            ...
            updateTimestamp.run(rep.timestamp, newId);
        });
        console.log(`Updated timestamps for ${clusters.length} concepts in ${sourceName}.`);
    });
...
async function runLocalAI() {
    console.log('Loading Local AI Model (Xenova Transformers) for clustering (this will take a few minutes)...');
